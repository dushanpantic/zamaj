...
A session can be started as a **deload**: each working exercise's planned set count is halved (rounded up, warmups untouched) in the frozen snapshot, so logging the lighter quota still reads as *completed*, not partial. Weight cuts stay manual. A deload is tagged for life — it carries a **DELOAD** badge on the live overview, the session review, and the recent-sessions list, is marked in plain-text export, and is held out of the top-set progress trend and the CAPPED computation so a deliberately easy week never reads as a regression or a maxed lift. The program template is never changed.
...
- **Workout-day picker** — [workout_day_picker_screen.dart](mobile/lib/modules/workout_day_picker/screens/workout_day_picker_screen.dart). Choose which day of the program to do today. A **Deload week** toggle (off by default, reset each visit) starts the chosen day as a deload. Only one session runs at a time: while any session is in progress (this program or another), a top banner offers to resume it and the other days recede with their start action removed, so resume is the one available action.

### In-session (sweaty-hands)
- **Workout overview** — [workout_overview_screen.dart](mobile/lib/modules/workout_overview/screens/workout_overview_screen.dart). The full session: every exercise card expands to its set list, where the active set logs in one tap on its circle (recording the suggested value, with an UNDO snackbar) or opens an inline ± editor to log a value that differs from plan; drag-to-reorder between cards, drag-onto-card (or a partner-picker sheet) to form a superset, reorder a whole superset as one block (drag its header handle into a gap, or its Move up / Move down), ungroup, end or skip an exercise (one adaptive action — Skip when nothing's logged, End when some sets are), log an extra set beyond plan on a completed exercise (its "Add set" kebab action — the exercise stays *completed* and the extra logs as work beyond the plan, never folded into the frozen snapshot), session notes, extra work, end session. Once a session is ended the screen locks new logs and structural changes, but already-logged set values stay editable so a mis-logged number can still be corrected; a summary card then heads the screen with the session's time, working sets done vs planned, and total weighted volume. The note, extra-work, and group-with-partner flows each open as a bottom sheet.
